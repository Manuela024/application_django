...
export const AuthProvider = ({ children }) => {
  ...
  // ⭐⭐⭐ NOUVELLES FONCTIONS PASSWORD RESET ⭐⭐⭐
  const sendPasswordResetCode = async (email) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/send-password-reset-code/`, { email })
      return { success: true, data: response.data }
    } catch (error) {
      return { 
        success: false, 
        error: error.response?.data?.error || 'Erreur lors de l\'envoi du code' 
      }
    }
  }
  ...
  const changePasswordAfterVerification = async (email, newPassword, confirmPassword) => {
    try {
      const response = await axios.post(`${API_BASE_URL}/auth/change-password-after-verification/`, {
        email,
